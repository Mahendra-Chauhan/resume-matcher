...
def extract_text(pdf_path):
    ...
    # Debug line — shows in your terminal so you can SEE what's actually
    # happening instead of guessing. Remove once you trust the behavior.
    print(f"[extractor] {pdf_path}: pdfplumber found {char_count} characters")

    # If we got barely any text, this is probably a scanned image PDF.
    # Raised from 50 -> 150 chars, since 50 was too easy to clear with
    # partial/junk text (e.g. a watermark or header line) on some scans.
    if char_count < 150:
        text = _extract_with_ocr(pdf_path)
        print(f"[extractor] {pdf_path}: OCR fallback found {len(text.strip())} characters")
        return text, True
